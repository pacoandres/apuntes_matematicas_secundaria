...
\question Escribe la ecuación de la mediatriz del segmento que tiene por extremos los puntos
  $A(2,-1)$ y $B(4, 5)$.
  \begin{solution}
    Este problema se puede resolver de dos maneras, dependiendo de el punto de vista desde el
    que definamos mediatriz.
    \begin{itemize}
    \item La primera forma es definiendo la mediatriz de la siguiente manera:
      \emph{``la mediatriz de un segmento es una
    ...
    \item La segunda manera es definiendo la mediatriz como \emph{la recta que contiene los
        puntos que equidistan de los extremos del segmento}.\\
      Entonces tenemos que encontrar los puntos que se encuentran a la misma distancia de $A(2,-1)$
      y $B(4, 5)$.\\
      Llamaremos $P(x,y)$ a uno de esos puntos.\\
      Recordamos que la distancia entre dos puntos es el módulo del vector que los une, con lo que
      para que las distancias a $A$ y $B$ sean iguales tiene que ocurrir que:
      \[|\overrightarrow{AP}| = |\overrightarrow{BP}|\]
      Calculamos los vectores:
      \begin{itemize}
      \item $\overrightarrow{AP} = (x-2, y + 1)$
      \item $\overrightarrow{BP} = (x-4, y -5)$
      \end{itemize}
      ...
      Desarrollamos:
      \[x^2 - 4x + 4 + y^2 + 2y + 1 = x^2 -8x + 16 +y^2 -10x +25\]
      Llevamos todo a la izquierda y reducimos:
      \[4x + 12y -36 = 0\]
      \[x + 3y -9 = 0\]
      Y ésta es la ecuación implícita de la mediatriz.
    \end{itemize}
    Para comprobar si hemos obtenido lo mismo vamos a pasar la ecuación continua que hemos obtenido
    con la primera forma para ver si coincide con la obtenida en la segunda forma (es más sencillo
    pasar de continua a implícita que al revés).\\
    Habíamos obtenido que la mediatriz tiene de ecuación continua:
    \[r: \frac{y -2}{1} = \frac{x-3}{-3}\]
    Quitamos denominadores:
    \[-3(y - 2) = 1*(x-3)\]
    Desarrollamos y lo llevamos todo a la izquierda:
    \[-x - 3y + 9 = 0\]
    Y vemos que son iguales si cambiamos los signos, con lo que ambos métodos son válidos para
    calcular la mediatriz.
  \end{solution}
...
\begin{solution}
  Siempre que tengamos un problema en el que nos dan tres puntos formando un triángulo hay que
  comprobar que forman un triángulo de verdad.\\
  Si tres puntos están alineados, en la misma recta, no pueden formar un triángulo. Con lo que
  tenemos que ver si están en la misma recta, y para esto vamos a utilizar los vectores que unen
  uno de los puntos con los otros dos, y si están en la misma línea tienen que ser proporcionales.
  \begin{itemize}
  \item $\overrightarrow{AB} = (3, -3)$
  \item $\overrightarrow{AC} = (1, 2)$
  \end{itemize}
  Y claramente \textbf{no son proporcionales} $\left(\frac{3}{1}\neq\frac{-3}{2}\right)$, con lo que \textbf{SÍ forman triángulo}.\\
  
  Una vez que hemos comprobado que es un triángulo vamos a recordar qué es exactamente la
  altura de un triángulo: \emph{``una altura de
...
\question Dada la recta
  ...
  \begin{cases}
    ...
    y=&1+3t
  \end{cases}$\\
  indica si los puntos $A(-2, 5)$ y $B(0, 7)$ pertenecen a la recta.
  \begin{solution}
    Hemos visto que si un punto pertenece a la recta tiene que verificar la ecuación, ya hemos
    hecho algún ejercicio similar.\\
    
    La diferencia es que en este caso la ecuación que nos dan es la paramétrica, que en realidad
    tiene dos ecuaciones y la manera de resolverlo cambia un poco.\\
    De hecho se puede resolver de dos maneras, y lo vamos a hacer de las dos para cada uno de los
    puntos por los que nos preguntan.
    \begin{itemize}
    \item \textbf{Primera forma: directamente}.\\
      Lo que vamos a hacer es sustituir los puntos en la ecuación de manera que \textbf{
        vamos a obtener unos valores para el parámetro} $t$. Si estos valores
      \textbf{son iguales} en las dos ecuaciones entonces \textbf{el punto pertenece a la recta}.
      Si \textbf{son distintos} el punto \textbf{no pertenece a la recta}.
      \begin{itemize}
      \item $A(-2,5)$\\
        Sustituimos en la paramétrica y obtenemos:
        \[
          \begin{cases}
            -2=&2 - t\\
            5=&1 + 3t
          ...
          \begin{cases}
            ...
            t=&\frac{4}{3}
          \end{cases}\]
        En cada ecuación obtenemos \textbf{un valor distinto para $t$}, por lo tanto el punto
        $A$ \textbf{no pertenece a la recta} $r$ ($A \not\in r$).
      \item $B(0, 7)$\\
        Sustituimos de el punto $B$:
        \[
          \begin{cases}
            0=&2 - t\\
            7=&1 + 3t
          ...
          \begin{cases}
            ...
            t=&2
          \end{cases}\]
        En este caso \textbf{las dos ecuaciones nos dan el mismo valor para $t$}, con lo que el
        punto $B$ \textbf{sí pertenece a la recta} $r$ ($B \in r$).
      \end{itemize}
      Esta forma es la más rápida, aunque a algunas personas les resulta compleja conceptualmente.
    \item \textbf{Segunda forma: pasando a otra ecuación}.\\
      Consiste en pasar a otra ecuación, por ejemplo la continua, y ver si obtenemos el mismo
      resultado a cada lado del igual. Es decir, es el mismo método que hemos utilizado en ejercicios
      anteriores.\\
      
      Vamos a pasar a la ecuación continua:
      \[\begin{cases}
        x=&2-t\\
        y=&1+3t
      \end{cases} \longrightarrow \begin{cases}
        t=&\frac{x-2}{-1}\\
        t=&\frac{y -1}{3}
      \end{cases} \longleftarrow \frac{x-2}{-1} = \frac{y - 1}{3}\]
    Y ahora comprobamos cada punto.
    \begin{itemize}
    \item $A(-2, 5)$\\
      Sustituimos y queda:
      \[\frac{-2-2}{-1} = \frac{5-1}{3}\]
      \[\frac{-4}{-1} = \frac{4}{3}\]
      Igualdad que no es cierta, por lo que $A \not\in r$.
    \item $B(0,7)$\\
      Sustituimos:
      \[\frac{0-2}{-1} = \frac{7-1}{3}\]
      \[\frac{-2}{-1} = \frac{6}{3}\]
      Y como esta igualdad sí es cierta tenemos que $B \in r$.
    \end{itemize}
    \end{itemize}
  \end{solution}
\end{questions}
